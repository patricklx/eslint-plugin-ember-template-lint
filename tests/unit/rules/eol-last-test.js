...
(0, _ruleTestHarness.default)({
  ...
  {
    config: 'always',
    template: ["import { hbs } from 'ember-cli-htmlbars';", '', "test('it renders', async (assert) => {", '  await render(hbs`<img>`);', ');'].join('\n'),
    meta: {
      filePath: 'layout.js'
    }
  }],
  ...
  */
  ...
  error: [{
    ...
    result: {
      fatal: true,
      message: 'allows setting the configuration to `"editorconfig"`, _only_ when an `.editorconfig` file with the `insert_final_newline` setting exists'
    }
  }, {
    config: true,
    template: 'test',
