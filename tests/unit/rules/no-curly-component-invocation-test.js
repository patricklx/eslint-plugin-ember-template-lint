...
var _curlyComponentInvocation = require("../../../lib/helpers/curly-component-invocation.js");
var _noCurlyComponentInvocation = require("../../../lib/rules/no-curly-component-invocation.js");
...
const SHARED_MOSTLY_GOOD = ['{{foo}}', '{{foo.bar}}', '{{model.selectedTransfersCount}}', '{{request.note}}'];
const SHARED_GOOD = ['{{#each items as |item|}}{{item}}{{/each}}', '{{#each items as |item|}}{{item.foo}}{{/each}}', '{{42}}', '{{true}}', '{{undefined}}', '{{"foo-bar"}}', '{{foo bar}}', '<div {{foo}} />', '<Foo @bar={{baz}} />', '{{#foo bar}}{{/foo}}', '{{#foo}}bar{{else}}baz{{/foo}}',
// single-word no-arguments built-ins
'{{array}}', '{{concat}}', '{{debugger}}', '{{has-block}}', '{{has-block-params}}', '{{hasBlock}}', '{{hash}}', '{{outlet}}', '{{unique-id}}', '{{yield}}', '{{yield to="inverse"}}',
// ember-cli related addon
'{{app-version}}', '{{app-version versionOnly=true}}',
// real world examples
'<GoodCode />', '<GoodCode></GoodCode>', '{{if someProperty "yay"}}', '<Nested::GoodCode />', '<Nested::GoodCode @someProperty={{-50}} @someProperty={{"-50"}} @someProperty={{true}} />', '{{some-valid-helper param}}', '{{some/valid-nested-helper param}}', '{{@someArg}}', '{{this.someProperty}}', '{{#-in-element destinationElement}}Hello{{/-in-element}}', '{{#in-element destinationElement}}Hello{{/in-element}}', '{{#some-component foo="bar"}}foo{{else}}bar{{/some-component}}', '<MyComponent @arg={{my-helper this.foobar}} />', '<MyComponent @arg="{{my-helper this.foobar}}" />', '<MyComponent {{my-modifier this.foobar}} />', '{{svg-jar "status"}}', '{{t "some.translation.key"}}', '{{#animated-if condition}}foo{{/animated-if}}'];
const SHARED_BAD = [{
  template: '{{foo-bar}}',
  results: [{
...
(0, _ruleTestHarness.default)({
  ...
  good: [...SHARED_MOSTLY_GOOD, ...SHARED_GOOD, '{{#each items as |disallowed|}}{{disallowed}}{{/each}}'],
  bad: [...SHARED_BAD, {
    template: '{{disallowed}}',
    results: [{
...
(0, _ruleTestHarness.default)({
  ...
  good: [...SHARED_MOSTLY_GOOD, ...SHARED_GOOD, '{{aaa-bbb}}', '{{aaa/bbb}}', '{{aaa-bbb bar=baz}}', '{{#aaa-bbb bar=baz}}{{/aaa-bbb}}'],
  bad: [...SHARED_BAD]
});
(0, _ruleTestHarness.default)({
  name: 'no-curly-component-invocation',
...
(0, _ruleTestHarness.default)({
  ...
  bad: [{
    ...
    template: '{{#foo-bar as |foo-baz|}}{{#foo-baz as |foo-boo|}}{{foo-boo}}{{/foo-baz}}{{/foo-bar}}',
    ...
    template: '{{#let (component "foo") as |my-component|}}{{#my-component}}{{/my-component}}{{/let}}',
    fixedTemplate: '{{#let (component "foo") as |my-component|}}<my-component></my-component>{{/let}}'
  }, {
    template: "{{#some-thing foo=(concat true null undefined 42 (my-helper value) 'lol-' something ' ' 'wtf')}}{{/some-thing}}",
    fixedTemplate: '<SomeThing @foo="truenullundefined42{{my-helper value}}lol-{{something}} wtf"></SomeThing>'
  }]
});
  ...
  describe('parseConfig', () => {
    ...
    for (let [input, expected] of TESTS) {
      test(`${JSON.stringify(input)} -> ${JSON.stringify(expected)}`, () => {
        expect((0, _noCurlyComponentInvocation.parseConfig)(input)).toEqual(expected);
      });
    }
    const FAILURE_TESTS = [undefined, null, false, 'error'];
    for (let input of FAILURE_TESTS) {
      test(`${JSON.stringify(input)} -> Error`, () => {
        expect(() => (0, _noCurlyComponentInvocation.parseConfig)(input)).toThrow();
      });
    }
  });
});