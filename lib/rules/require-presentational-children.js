...
function getAllElementNodeDescendants(node) {
  return _astNodeInfo.childrenFor(node).flatMap(childNode => {
    const isNotPresentationRole = _astNodeInfo.attributeTextValue(_astNodeInfo.findAttribute(childNode, 'role')) !== 'presentation';
    let descendants = [];
    if (_astNodeInfo.hasChildren(childNode)) {
...
'svg']);
class RequirePresentationalChildren extends _base {
  parseConfig(config) {
    return {
  ...
  visitor() {
    return {
      GlimmerElementNode(node) {
        const roleAttrValue = _astNodeInfo.attributeTextValue(_astNodeInfo.findAttribute(node, 'role'));
