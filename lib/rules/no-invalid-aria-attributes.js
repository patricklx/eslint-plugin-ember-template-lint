...
const VALID_ARIA_ATTRIBUTES = _ariaQuery.aria;
...
function validityCheck(expectedType, permittedValues, allowUndefined, value) {
  ...
  switch (expectedType) {
    ...
    case 'idlist':
      {
        return typeof value === 'string' && value.split(' ').every(token => validityCheck('id', [], false, token));
      }
    case 'tristate':
      {
    ...
    case 'tokenlist':
      {
        return typeof value === 'string' && value.split(' ').every(token => permittedValues.includes(token.toLowerCase()));
      }
  }
}
class NoInvalidAriaAttributes extends _base.default {
  visitor() {
    return {
      GlimmerElementNode(node) {
        ...
        for (let attribute of foundAriaAttributes) {
          ...
          if (attribute.value.type === 'GlimmerMustacheStatement' || attribute.value.type === 'GlimmerConcatStatement') {
            return;
          } else {
            isValidValue = validityCheck(expectedType, permittedValues, allowUndefined, attribute.value.chars);
          }
          if (!isValidValue) {
            this.log({
              message: createInvalidAttributeTypeErrorMessage(attribute.name, expectedType, permittedValues),
              node
            });
          }
        }
...
exports.default = NoInvalidAriaAttributes;