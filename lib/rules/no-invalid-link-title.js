...
class NoInvalidLinkTitle extends _base.default {
  visitor() {
    return {
      GlimmerElementNode(node) {
        if (node.tag === 'a' || node.tag === 'LinkTo') {
          let hasTitleAttr = _astNodeInfo.default.hasAttribute(node, 'title');
          let titleAttr, titleAttrValue;
          if (hasTitleAttr) {
            titleAttr = _astNodeInfo.default.findAttribute(node, 'title');
          ...
          let hasTitleArg = _astNodeInfo.default.hasAttribute(node, '@title');
          let titleArg, titleArgValue;
          if (hasTitleArg) {
            titleArg = _astNodeInfo.default.findAttribute(node, '@title');
