...
class InlineLinkTo extends _base {
  visitor() {
    return {
      GlimmerMustacheStatement(node, {
        ...
        if (node.path.original === 'link-to') {
          ...
          if (this.mode === 'fix' && isFixable) {
            ...
            _replaceNode(node, parentNode, parentKey, _emberTemplateRecast.builders.block(node.path, node.params.slice(1), node.hash, _emberTemplateRecast.builders.blockItself([newBody])));
          } else {
            this.log({
              message,
              node,
