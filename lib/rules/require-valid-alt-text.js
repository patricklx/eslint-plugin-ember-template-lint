...
function hasAccessibleChild(node) {
  return _astNodeInfo.hasChildren(node);
}
...
class RequireValidAltText extends _base {
  logNode({
    node,
    message
  }) {
  ...
  visitor() {
    return {
      GlimmerElementNode(node) {
        if (_astNodeInfo.hasAttribute(node, 'hidden')) {
          return;
        }
        let hasAriaHiddenAttr = _astNodeInfo.hasAttribute(node, 'aria-hidden');
        if (hasAriaHiddenAttr) {
          let ariaHiddenAttr = _astNodeInfo.findAttribute(node, 'aria-hidden');
          if (ariaHiddenAttr.value.type === 'GlimmerTextNode' && ariaHiddenAttr.value.chars === 'true') {
            return;
          }
        }
        if (_astNodeInfo.hasAttribute(node, '...attributes')) {
          return;
        }
        ...
        if (isImg) {
          const hasAltAttribute = _astNodeInfo.hasAttribute(node, 'alt');
          let altAttribute, altValue;
          if (hasAltAttribute) {
            altAttribute = _astNodeInfo.findAttribute(node, 'alt');
          ...
          const hasRole = _astNodeInfo.hasAttribute(node, 'role');
          ...
          let hasSrcAttr = _astNodeInfo.hasAttribute(node, 'src');
          let srcAttr, srcValue;
          if (hasSrcAttr) {
            srcAttr = _astNodeInfo.findAttribute(node, 'src');
          ...
          if (!hasAltAttribute) {
            ...
            const altAttribute = _astNodeInfo.findAttribute(node, 'alt');
          ...
          const hasValidAttributes = _astNodeInfo.hasAnyAttribute(node, ['aria-label', 'aria-labelledby', 'alt']);
          ...
          const hasValidAttributes = _astNodeInfo.hasAnyAttribute(node, ['aria-label', 'aria-labelledby', 'title']);
          ...
          if (!_astNodeInfo.hasAnyAttribute(node, ['aria-label', 'aria-labelledby', 'alt'])) {
            this.logNode({
