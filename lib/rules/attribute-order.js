...
exports.createAttributesOrderErrorMessage = createAttributesOrderErrorMessage;
...
var _isValidConfigObject = require('../helpers/is-valid-config-object.js');
var _base = require('./_base.js');
const ERROR_MESSAGE = 'Your config does not match the allowed values';

// The parts of an GlimmerElementNode or GlimmerMustacheStatement that this rule is concerned with
...
function createAttributesOrderErrorMessage(config) {
  return _createErrorMessage('attribute-order', ERROR_MESSAGE, config);
}
...
class AttributeOrder extends _base {
  ...
  parseConfig(config) {
    ...
    if (!_isValidConfigObject(config, DEFAULT_CONFIG)) {
    ...
    if (typeof config === 'object') {
      if (config.order && ![TokenType.ARGUMENTS, TokenType.ATTRIBUTES, TokenType.MODIFIERS].every(attribute => config.order.includes(attribute))) {
        throw new Error(errorMessage);
      }
      return {
        ...DEFAULT_CONFIG,
  ...
  areSplattributesSurrounded(tokenGroups) {
    ...
    const maxIndex = Math.max(...Object.values(tokenGroups).map(tokenGroup => tokenGroup.lastIndex || Number.NEGATIVE_INFINITY));
    const minIndex = Math.min(...Object.values(tokenGroups).map(tokenGroup => tokenGroup.firstIndex || Number.POSITIVE_INFINITY));
  ...
  getAppliedOrder(tokenGroups) {
    ...
    const maxLastIndex = Math.max(...Object.values(tokenGroups).map(tokenGroup => tokenGroup.lastIndex).filter(i => Number.isInteger(i)));
    const minLastIndex = Math.min(...Object.values(tokenGroups).map(tokenGroup => tokenGroup.lastIndex).filter(i => Number.isInteger(i)));
    const orderMinusSplattributes = this.config.order.filter(attribute => attribute !== TokenType.SPLATTRIBUTES);
    const splattributeLastIndex = tokenGroups[TokenType.SPLATTRIBUTES] ? tokenGroups[TokenType.SPLATTRIBUTES].lastIndex : undefined;
    const attributeLastIndex = tokenGroups[TokenType.ATTRIBUTES] ? tokenGroups[TokenType.ATTRIBUTES].lastIndex : undefined;
    if (splattributeLastIndex === maxLastIndex || splattributeLastIndex && splattributeLastIndex > attributeLastIndex) {
      this.appliedOrder = [...orderMinusSplattributes, TokenType.SPLATTRIBUTES];
    } else if (splattributeLastIndex === minLastIndex || splattributeLastIndex && splattributeLastIndex < attributeLastIndex) {
  ...
  getTokenCategory(tokenType) {
    return [TokenType.ARGUMENTS, TokenType.ATTRIBUTES, TokenType.SPLATTRIBUTES].includes(tokenType) ? TokenType.ATTRIBUTES : tokenType;
  }
  ...
  getTokenType(node) {
    ...
    return node.name.startsWith('@') ? TokenType.ARGUMENTS : node.name.startsWith('...') ? TokenType.SPLATTRIBUTES : TokenType.ATTRIBUTES;
  }
  ...
  makeToken({
    ...
    case TokenType.MODIFIERS: {
      return _emberTemplateRecast.builders.elementModifier(_emberTemplateRecast.builders.path(node.path), node.params, node.hash, loc);
    }
    case TokenType.COMMENTS: {
      return node.type === 'GlimmerMustacheCommentStatement' ? _emberTemplateRecast.builders.mustacheComment(node.value, loc) : _emberTemplateRecast.builders.comment(node.value, loc);
    }
    default: {
      return _emberTemplateRecast.builders.attr(node.name, node.value, loc);
    }
    }
  }
  ...
  findUnorderedToken(tokenGroup) {
    return tokenGroup.items.find(item => this.calculateAttributeIndex(item) === tokenGroup.lastIndex);
  }
  ...
  createUnorderedErrorMessage(tokenGroups, tokenType, source) {
    ...
    if (order === 0) {
      const otherAttributes = `${this.getAppliedOrder(tokenGroups)[1]} and ${this.getAppliedOrder(tokenGroups)[2]}`;
      return `${this.capitalizedAttribute(tokenType)} ${source} must go before ${otherAttributes}`;
    }
    if (order === 1) {
      return `${this.capitalizedAttribute(tokenType)} ${source} must go after ${this.config.order[order - 1]}`;
    }
    return `${this.capitalizedAttribute(tokenType)} ${source} must go after ${this.getAppliedOrder(tokenGroups)[order - 1]}`;
  }
  ...
  isAttributeUnordered(tokenGroups, tokenType) {
    ...
    switch (order) {
    case 0: {
      return tokenGroups[tokenType].lastIndex > tokenGroups[this.getAppliedOrder(tokenGroups)[1]].firstIndex || tokenGroups[tokenType].lastIndex > tokenGroups[this.getAppliedOrder(tokenGroups)[2]].firstIndex || tokenGroups[tokenType].lastIndex > tokenGroups[this.getAppliedOrder(tokenGroups)[3]].firstIndex;
    }
    case 1: {
      return tokenGroups[tokenType].lastIndex < tokenGroups[this.getAppliedOrder(tokenGroups)[0]].firstIndex || tokenGroups[tokenType].lastIndex > tokenGroups[this.getAppliedOrder(tokenGroups)[2]].firstIndex;
    }
    case 2: {
      return tokenGroups[tokenType].lastIndex < tokenGroups[this.getAppliedOrder(tokenGroups)[1]].lastIndex || tokenGroups[tokenType].lastIndex > tokenGroups[this.getAppliedOrder(tokenGroups)[3]].lastIndex;
    }
    case 3: {
      return tokenGroups[tokenType].lastIndex < tokenGroups[this.getAppliedOrder(tokenGroups)[2]].lastIndex;
    }
    default: {
      break;
    }
  ...
  checkMustacheAttributesOrder(node) {
    ...
    const tokenGroups = this.makeTokenGroups(this.groupTokens(this.getMustacheStatementTokens(node, this.sourceForNode(node))));
    ...
    if (this.config.alphabetize && tokenGroup.unalphabetizedItemIndex >= 0) {
      ...
      this.log({
        message: this.createNotAlphabetizedErrorMessage(TokenType.ATTRIBUTES, this.sourceForNode(item)),
        node: item,
        fix: (fixer) => {
          const sorted = [...node.hash.pairs].sort((a, b) => a.key > b.key ? 1 : -1);
          return node.hash.pairs.map((p, i) => {
            if (p === sorted[i]) return ;
            const replaceWith = this.context.getSourceCode().text.slice(...sorted[i].range);
            return fixer.replaceTextRange(p.range, replaceWith);
          }).filter(x =>  !!x);
        }
      });
    }
  ...
  checkElementAttributesOrder(node, {
    ...
    const tokenGroups = this.makeTokenGroups(this.groupTokens(this.getElementNodeTokens(node, this.sourceForNode(node))));
    const sorted = [];
    const original = [];
    const messages = [];
    for (const tokenType of this.config.order) {
      ...
      if (this.config.alphabetize && tokenGroups[tokenType].unalphabetizedItemIndex >= 0 && !this.areSplattributesSurrounded(tokenGroups)) {
        const item = this.findUnalphabetizedToken(tokenGroups[tokenType]);
        const message = this.createNotAlphabetizedErrorMessage(tokenType, this.sourceForNode(item));
        messages.push(message);
        original.push(...node[nodeType]);
        const ordered = this.alphabetizeAttribute({
          tokenType,
          tokenGroups,
          node,
          parentNode,
      ...
      if (isAttributeUnordered) {
        ...
        const message = this.createUnorderedErrorMessage(tokenGroups, tokenType, this.sourceForNode(item));
        messages.push(message);
        original.push(...node[nodeType]);
        const ordered = this.orderAttribute({
          tokenType,
          tokenGroups,
          node,
          parentNode,
    ...
    this.log({
      ...
      fix: (fixer) => {
        return original.map((o, i) => {
          const replaceWith = this.context.getSourceCode().text.slice(...sorted[i].range);
          return fixer.replaceTextRange(o.range, replaceWith);
        });
      }
    });
  }
  ...
  alphabetizeAttribute({
    ...
    if (nodeType === TokenType.MODIFIERS || nodeType === TokenType.COMMENTS) {
      const ordered = [...node[nodeType]].sort((a, b) => a.path.original > b.path.original ? 1 : -1);
      return ordered;
    } else if (nodeType !== TokenType.MODIFIERS) {
      const ordered = [...node[nodeType]].sort((a, b) => {
  ...
  orderComment({
    ...
    for (const comment of reverseComments) {
      ...
      const fromIndex = this.getAllIndexes(tokenGroups).indexOf(this.calculateAttributeIndex(token));
      const fromPosInSorted = sorted.findIndex(node => this.calculateAttributeIndex(node) === this.getAllIndexes(tokenGroups)[fromIndex]);
  ...
  orderAttribute({
    ...
    const attributesModifiers = [...node[TokenType.ATTRIBUTES], ...node[TokenType.MODIFIERS], ...node[TokenType.COMMENTS]];
  ...
  visitor() {
    return {
      ...
      GlimmerElementNode(node, {
        ...
        this.checkElementAttributesOrder(node, {
          parentNode,
          parentKey
        });
      }
...
exports.default = AttributeOrder;
AttributeOrder.createAttributesOrderErrorMessage = exports.createAttributesOrderErrorMessage;
