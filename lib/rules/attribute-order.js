...
var _isValidConfigObject = _interopRequireDefault(require("../helpers/is-valid-config-object.js"));
...
function createAttributesOrderErrorMessage(config) {
  return (0, _createErrorMessage.default)('attribute-order', ERROR_MESSAGE, config);
}
...
class AttributeOrder extends _base.default {
  appliedOrder = undefined;
  parseConfig(config) {
    const errorMessage = createAttributesOrderErrorMessage(config);
    if (!(0, _isValidConfigObject.default)(config, DEFAULT_CONFIG)) {
  ...
  alphabetizeAttribute({
    ...
    if (this.mode === 'fix') {
      ...
      if (nodeType === TokenType.MODIFIERS || nodeType === TokenType.COMMENTS) {
        newNode = _emberTemplateRecast.builders.element(node.tag, {
          attrs: node[TokenType.ATTRIBUTES],
          modifiers: node[nodeType].sort((a, b) => a.path.original > b.path.original ? 1 : -1).map(node => _emberTemplateRecast.builders.elementModifier(_emberTemplateRecast.builders.path(node.path), node.params, node.hash)),
        ...
        const attrs = node[nodeType].sort((a, b) => {
        ...
        }).map(attr => _emberTemplateRecast.builders.attr(attr.name, attr.value));
        newNode = _emberTemplateRecast.builders.element(node.tag, {
          attrs,
  ...
  orderAttribute({
    ...
    if (this.mode === 'fix') {
      ...
      const {
        ...
        const loc = _emberTemplateRecast.builders.loc(line, startColumn, line, endColumn);
        const newAttribute = this.makeToken({
          nodeType,
          node: currAttr,
          loc
        });
        if (newAttribute.type === 'GlimmerAttrNode') {
      ...
      const newNode = _emberTemplateRecast.builders.element(node.tag, {
        attrs: attributes,
        modifiers,
        children: node.children,
        blockParams: node.blockParams,
