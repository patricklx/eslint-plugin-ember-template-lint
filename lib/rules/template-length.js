...
class TemplateLength extends _base.default {
  parseConfig(config) {
    ...
    let errorMessage = (0, _createErrorMessage.default)(this.ruleName, ['  * boolean - `true` to enable / `false` to disable', '  * object -- An object with the following keys:', '    * `min` -- Minimum length of a template', '    * `max` -- Maximum length of a template'], config);
      ...
      GlimmerTemplate: {
        ...
        exit(node) {
          if (this.config.max && this.source.length > this.config.max) {
            this.log({
              message: `GlimmerTemplate length of ${this.source.length} exceeds ${this.config.max}`,
              node
            });
          } else if (this.config.min && this.source.length < this.config.min) {
            this.log({
              message: `GlimmerTemplate length of ${this.source.length} is smaller than ${this.config.min}`,
              node
            });
          }
        }
