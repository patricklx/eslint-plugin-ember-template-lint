...
class TemplateLength extends _base {
  parseConfig(config) {
    ...
    let errorMessage = _createErrorMessage(this.ruleName, ['  * boolean - `true` to enable / `false` to disable', '  * object -- An object with the following keys:', '    * `min` -- Minimum length of a template', '    * `max` -- Maximum length of a template'], config);
