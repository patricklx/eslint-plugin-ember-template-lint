...
class EolLast extends _base.default {
  parseConfig(config) {
    ...
    let errorMessage = (0, _createErrorMessage.default)(this.ruleName, ['  * "always" - enforces that files end with a newline', '  * "editorconfig" - requires or disallows final newlines based your projects `.editorconfig` settings (via `insert_final_newline`)', '  * "never" - enforces that files do not end with a newline'], config);
