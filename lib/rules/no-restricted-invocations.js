...
class NoRestrictedInvocations extends _base {
  parseConfig(config) {
    switch (typeof config) {
      ...
      case 'object':
        {
          if (Array.isArray(config) && config.length > 0 && config.every(item => typeof item === 'string' && _isDasherizedComponentOrHelperName(item) || typeof item === 'object' && Array.isArray(item.names) && item.names.length > 0 && item.names.every(_isDasherizedComponentOrHelperName) && isValidCustomErrorMessage(item.message))) {
            return config;
          }
          break;
        }
  ...
  _getComponentOrHelperName(node) {
    ...
    if (node.type === 'GlimmerElementNode') {
      ...
      return _dasherizeComponentName(node.tag);
    } else {
