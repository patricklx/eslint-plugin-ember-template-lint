...
exports.createTableGroupsErrorMessage = createTableGroupsErrorMessage;
...
var _isDasherizedComponentOrHelperName = require("../helpers/is-dasherized-component-or-helper-name.js");
...
function getEffectiveChildren(node) {
  return _astNodeInfo.childrenFor(node).flatMap(childNode => {
    if (_astNodeInfo.isControlFlowHelper(childNode)) {
    ...
    case 'GlimmerElementNode':
      {
        ...
        const possibleIndices = config.get(_dasherizeComponentName(node.tag)) || [];
        if (possibleIndices.length > 0) {
          return {
            allowed: true,
            indices: possibleIndices
...
function createTableGroupsErrorMessage(ruleName, config) {
  return _createErrorMessage(ruleName, ['  One of these:', '  * boolean - `true` to enable / `false` to disable', '  * object[] - with the following keys:', '    * `allowed-caption-components` - string[] - components to treat as having the caption tag (using kebab-case names like `nested-scope/component-name`)', '    * `allowed-colgroup-components` - string[] - components to treat as having the colgroup tag (using kebab-case names like `nested-scope/component-name`)', '    * `allowed-thead-components` - string[] - components to treat as having the thead tag (using kebab-case names like `nested-scope/component-name`)', '    * `allowed-tbody-components` - string[] - components to treat as having the tbody tag (using kebab-case names like `nested-scope/component-name`)', '    * `allowed-tfoot-components` - string[] - components to treat as having the tfoot tag (using kebab-case names like `nested-scope/component-name`)'], config);
}
class TableGroups extends _base {
  parseConfig(config) {
    ...
    switch (configType) {
      ...
      case 'object':
        {
          const allowedComponentKeysByIndex = ['allowed-caption-components', 'allowed-colgroup-components', 'allowed-thead-components', 'allowed-tbody-components', 'allowed-tfoot-components'];
          const result = new Map();
          let isValid = true;
          for (const [index, key] of allowedComponentKeysByIndex.entries()) {
            if (key in config) {
              const allowedComponents = config[key];
              if (Array.isArray(allowedComponents) && allowedComponents.every(_isDasherizedComponentOrHelperName)) {
        ...
        if (node.tag === 'table') {
          ...
          for (const child of children) {
            const {
              allowed,
              indices
            } = isAllowedTableChild(child, this.config);
            if (!allowed) {
              this.log({
                message,
                node
            ...
            if (indices.length > 0) {
              const newAllowedMinimumIndices = new Set([...currentAllowedMinimumIndices].flatMap(currentIndex => {
