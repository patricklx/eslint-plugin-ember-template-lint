...
function getEffectiveChildren(node) {
  return _astNodeInfo.default.childrenFor(node).flatMap(childNode => {
    if (_astNodeInfo.default.isControlFlowHelper(childNode)) {
    ...
    case 'GlimmerElementNode':
      {
        ...
        const possibleIndices = config.get((0, _dasherizeComponentName.default)(node.tag)) || [];
        if (possibleIndices.length > 0) {
          return {
            allowed: true,
            indices: possibleIndices
      ...
      case 'object':
        {
          ...
          for (const [index, key] of allowedComponentKeysByIndex.entries()) {
            if (key in config) {
              ...
              if (Array.isArray(allowedComponents) && allowedComponents.every(_isDasherizedComponentOrHelperName.default)) {
