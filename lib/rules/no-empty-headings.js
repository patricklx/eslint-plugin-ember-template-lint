...
function hasAllowedNode(nodes, scope) {
  for (const node of nodes) {
    ...
    if (type === 'GlimmerElementNode') {
      if ((0, _isAngleBracketComponent.default)(scope, node)) {
        return true;
      }
      if (!isHidden(node) && _astNodeInfo.default.hasChildren(node)) {
        if (hasAllowedNode(_astNodeInfo.default.childrenFor(node), scope)) {
          return true;
        }
      }
    }
...
class NoEmptyHeadings extends _base.default {
  visitor() {
    return {
      GlimmerElementNode(node) {
        const role = _astNodeInfo.default.findAttribute(node, 'role');
        const hasHeadingRole = role && role.value.chars === 'heading';
        if (HEADINGS.has(node.tag) || hasHeadingRole) {
          if (isHidden(node)) {
            return;
          }
          if (!_astNodeInfo.default.hasChildren(node)) {
            this.log({
          ...
          const childNodes = _astNodeInfo.default.childrenFor(node);
