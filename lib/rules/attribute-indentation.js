...
function getEndLocationForOpen(node) {
  ...
  } else if (node.type === 'GlimmerElementNode' && node.children.length > 0) {
...
class AttributeIndentation extends _base.default {
  ...
  getBlockParamStartLoc(node) {
    ...
    const actualProgramStartLine = /^\s*}}/.test(this.source[node.program.loc.start.line - 1]) ? 1 : 0;
    ...
    if (node.params.length === 0 && node.hash.pairs.length === 0) {
      ...
      if (paramOrHashPairEndLoc.line === programStartLoc.line) {
        ...
        */
        ...
        let hashPairLineEndSource = this.sourceForNode({
          loc
        }).trim();
  ...
  validateBlockParams(node) {
    ...
    */
    ...
    const expectedColumnNextLocation = node.type === 'GlimmerElementNode' && !node.selfClosing ? 1 : 2;
    return {
  ...
  iterateParams(params, type, expectedLineStart, expectedColumnStart, node) {
    ...
    for (const param of params) {
      ...
      if (expectedLineStart !== actualStartLocation.line || expectedColumnStart !== actualStartLocation.column) {
        let paramName = param[namePath] || param.path.original;
  ...
  validateCloseBrace(node, nextLocation) {
    ...
    */
    ...
    let end = getEndLocationForOpen(node);
    const actualColumnStartLocation = node.type === 'GlimmerElementNode' && !node.selfClosing ? 1 : node.type === 'GlimmerMustacheStatement' && !node.escaped ? 3 : 2;
    const actualStartLocation = {
      line: end.line,
      column: end.column - actualColumnStartLocation
    };
    const endPosition = node.type === 'GlimmerElementNode' ? this.config.elementOpenEnd : this.config.mustacheOpenEnd;
    ...
    let componentName = node.type === 'GlimmerElementNode' ? node.tag : node.path.original;
    if (actualStartLocation.line !== expectedStartLocation.line || actualStartLocation.column !== expectedStartLocation.column) {
  ...
  validateClosingTag(node, expectedStartLine) {
    ...
    */
    ...
    let tagName = node.type === 'GlimmerElementNode' ? node.tag : node.path.original;
    if (actualStartLocation.line !== expectedStartLocation.line || actualStartLocation.column !== expectedStartLocation.column) {
  ...
  parseConfig(config) {
    ...
    let errorMessage = (0, _createErrorMessage.default)(this.ruleName, ['  * boolean - `true` - Enables the rule to be enforced when the opening invocation has more than 80 characters or when it spans multiple lines.', '  * { open-invocation-max-len: n characters, indentation: m  } - n : The max length of the opening invocation can be configured', '  *                                                            - m : The desired indentation of attribute', '  * { process-elements: `boolean` } - `true` : Also parse HTML/SVG attributes', '  * { element-open-end: `new-line`|`last-attribute` } - Enforce the position of the closing brace `>` to be on a new line or next to the last attribute (defaults to `new-line`)', '  * { mustache-open-end: `new-line`|`last-attribute` } - Enforce the position of the closing braces `}}` to be on a new line or next to the last attribute (defaults to `new-line`)'], config);
        ...
        if (this.config.processElements) {
          if (canApplyRule(node, node.type, this.config)) {
            ...
            if (node.children.length) {
              ...
              const expectedStartLine = lastChild.type === 'GlimmerBlockStatement' ? lastChild.loc.end.line + 1 : lastChild.loc.end.line;
...
exports.default = AttributeIndentation;