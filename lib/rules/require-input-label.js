...
function hasValidLabelParent(path, config) {
  ...
  if (labelParentPath && _astNodeInfo.childrenFor(labelParentPath.node).length > 1) {
    return true;
  }
  return false;
}
...
class RequireInputLabel extends _base {
  parseConfig(config) {
    ...
    let errorMessage = _createErrorMessage(this.ruleName, ['  * boolean - `true` to enable / `false` to disable', '  * object -- An object with the following keys:', '    * `labelTags` -- An array of component / helper names for that may be called without arguments'], config);
  ...
  visitor() {
    return {
      GlimmerElementNode(node, path) {
        ...
        const typeAttribute = _astNodeInfo.findAttribute(node, 'type');
        ...
        let attributes = validAttributesList.filter(name => _astNodeInfo.hasAttribute(node, name));
        labelCount += attributes.length;
        if (labelCount === 1) {
          return;
        }
        if (hasValidLabelParent(path, this.config) && _astNodeInfo.hasAttribute(node, 'id')) {
          return;
        }
