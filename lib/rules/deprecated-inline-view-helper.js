...
'The inline form of `view` is deprecated. Please use the `Ember.GlimmerComponent` instead. ' + 'See the deprecation guide at ' + DEPRECATION_URL;
...
function manageMustacheViewInvocation(context, node, isBlockStatement) {
  ...
  const isYield = node.path.original === 'yield' && node.path.this === false && node.path.data === false;
  if (pairs.length > 0) {
    for (const currentPair of pairs) {
      let originalValue = currentPair.value.original;
      if (typeof originalValue === 'string' && originalValue.split('.')[0] === 'view' && !isYield && currentPair.key !== 'to') {
        if (isBlockStatement) {
...
class DeprecatedInlineViewHelper extends _base.default {
  visitor() {
    return {
      ...
      GlimmerMustacheStatement(node) {
        // this MustachStatement was already processed by GlimmerElementNode
        if (node._processedByInlineViewHelper === true) {
          return;
        }
        if (node.path.type === 'GlimmerPathExpression' && node.path.parts[0] === 'view') {
          if (node.params.length === 1) {
            this.processWithArgument(node);
          } else if (node.loc.start.line !== null && !node.path.this && !node.path.data && !this.scope.isLocal(node)) {
...
exports.default = DeprecatedInlineViewHelper;