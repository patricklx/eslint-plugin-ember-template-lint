...
function deprecateArgument(componentName, argumentName, replacementAttribute) {
  ...
  if (replacementAttribute) {
    msgs.push(`Instead, please pass the attribute directly, i.e. "<${componentName} ${replacementAttribute}={{...}} />" instead of "<${componentName} ${argumentName}={{...}} />".`);
  }
  return msgs.join('\n');
}
function deprecateEvent(componentName, argumentName, replacementAttribute) {
  const msgs = [`Passing the "${argumentName}" argument to <${componentName} /> is deprecated.`];
  if (replacementAttribute) {
    msgs.push(`Instead, please use the {{on}} modifier, i.e. "<${componentName} {{on "${replacementAttribute}" ...}} />" instead of "<${componentName} ${argumentName}={{...}} />".`);
  }
  return msgs.join('\n');
}
...
function fix(node, argument) {
  ...
  if (replacementEvent && argument.value.type === 'GlimmerMustacheStatement') {
    let expr;
    if (argument.value.params.length > 0) {
      expr = _emberTemplateRecast.builders.sexpr(argument.value.path, argument.value.params);
    } else {
      expr = argument.value.path;
    }
    const modifier = _emberTemplateRecast.builders.elementModifier('on', [_emberTemplateRecast.builders.string(replacementEvent), expr]);
...
function ERROR_MESSAGE(tagName, argumentName) {
  ...
  const deprecatedArgs = tagMeta.deprecatedArguments || {};
  const deprecatedEvents = tagMeta.deprecatedEvents || {};
  const candidates = [...new Set([...tagMeta.arguments, ...Object.keys(deprecatedArgs).map(e => removeAtSymbol(e)), ...Object.keys(deprecatedEvents).map(e => removeAtSymbol(e))])];
  ...
  while (!fuzzyResults.length && query.length) {
    fuzzyResults = new _fuse.default(candidates).search(query);
...
function REQUIRED_MESSAGE(tagName, argumentNames) {
  return `Argument${argumentNames.length > 1 ? 's' : ''} ${argumentNames.map(el => `"@${el}"`).join(' or ')} is required for <${tagName} /> component.`;
}
function CONFLICT_MESSAGE(argumentName, rawList) {
  const conflictsList = rawList.filter(el => `@${el}` !== argumentName);
  return `"${argumentName}" conflicts with ${conflictsList.map(el => `"@${el}"`).join(', ')}, only one should exists.`;
}
...
class NoUnknownArgumentsForBuiltinComponents extends _base.default {
  visitor() {
    return {
...
exports.default = NoUnknownArgumentsForBuiltinComponents;