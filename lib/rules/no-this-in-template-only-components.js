...
function isTemplateOnlyComponent(hbsFilePath, validComponentExtensions) {
  ...
  function componentClassExists(pathWithoutExtension) {
    return validComponentExtensions.some(ext => _nodeFs.default.existsSync(pathWithoutExtension + ext));
  }
  const paths = _nodePath.default.normalize(hbsFilePath).split(_nodePath.default.sep);
  if (paths[0] === 'app' || paths[0] === 'addon') {
    if (paths[1] === 'templates') {
      if (paths[2] === 'components') {
        ...
        const moduleName = _nodePath.default.basename(hbsFilePath, '.hbs');
        const classFilePath = _nodePath.default.join(paths[0], 'components', ...paths.slice(3, -1), moduleName);
      ...
      const moduleName = _nodePath.default.basename(hbsFilePath, '.hbs');
      const classFilePath = _nodePath.default.join(_nodePath.default.dirname(hbsFilePath), moduleName);
      return !componentClassExists(classFilePath);
    }
  }
...
class NoThisInTemplateOnlyComponents extends _base.default {
  parseConfig(config) {
