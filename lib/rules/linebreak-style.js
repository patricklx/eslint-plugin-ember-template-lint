...
class LinebreakStyle extends _base {
  constructor(options) {
    super(options);
  ...
  parseConfig(config) {
    ...
    let errorMessage = _createErrorMessage(this.ruleName, ['* boolean -- `true` for enforcing consistency (all `CRLF` or all `LF` not both in a single file)', '* string -- `system` for the current platforms default line ending / `unix` for LF linebreaks / `windows` for CRLF linebreaks'], config);
