...
class SimpleUnless extends _base.default {
  parseConfig(config) {
  ...
  visitor() {
    return {
      ...
      GlimmerBlockStatement(node) {
        ...
        const fixMode = this.mode === 'fix';
        if (nodeInverse) {
          if (_astNodeInfo.default.isUnless(node)) {
            if (nodeInverse.body[0] && _astNodeInfo.default.isIf(nodeInverse.body[0])) {
        ...
        } else if (_astNodeInfo.default.isUnless(node) && node.params[0].path) {
          this._withHelper(node, fixMode);
        }
      }
    };
