...
function hasValidAriaLabel(node) {
  const ariaLabelledbyAttr = _astNodeInfo.findAttribute(node, 'aria-labelledby');
  if (ariaLabelledbyAttr) {
    let ariaLabelledby = getTrimmedText(ariaLabelledbyAttr.value.chars);
    return ariaLabelledby.length > 0;
  }
  const ariaLabelAttr = _astNodeInfo.findAttribute(node, 'aria-label');
  if (ariaLabelAttr) {
...
class NoInvalidLinkText extends _base {
  parseConfig(config) {
