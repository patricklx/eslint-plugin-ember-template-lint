...
var _camelize = _interopRequireDefault(require("../helpers/camelize.js"));
...
const FORMAT_METHOD = {
  [FORMAT.CAMEL_CASE]: _camelize.default,
  [FORMAT.KEBAB_CASE]: _dasherizeComponentName.default
};
const FORMATS = Object.values(FORMAT);
const DEFAULT_FORMAT = FORMAT.CAMEL_CASE;
class RequireValidNamedBlockNamingFormat extends _base.default {
  parseConfig(config) {
    ...
    let errorMessage = (0, _createErrorMessage.default)(this.ruleName, FORMATS.map(format => `  * "${format}" - Requires the use of the "${format}" naming format.`), config);
