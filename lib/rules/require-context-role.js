...
class RequireContextRole extends _base.default {
  visitor() {
    return {
      GlimmerElementNode(node, path) {
        ...
        while (parentPath && parentPath.parent && parentElementIndex === 0) {
          ...
          if (parentNode && parentNode.type === 'GlimmerElementNode') {
            if (_astNodeInfo.default.hasAttribute(parentNode, 'aria-hidden')) {
              return;
            }
            const parentRole = _astNodeInfo.default.findAttribute(parentNode, 'role');
