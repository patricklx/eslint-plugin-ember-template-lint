...
class NoNegatedCondition extends _base.default {
  parseConfig(config) {
  ...
  visitor() {
    ...
    function checkNode(node) {
      const isIf = _astNodeInfo.default.isIf(node);
      const isUnless = _astNodeInfo.default.isUnless(node);
      const {
        simplifyHelpers
      } = this.config;
      if (!isIf && !isUnless) {
      ...
      if (node.type === 'GlimmerBlockStatement') {
        ...
        if (node.inverse && node.inverse.body.length > 0 && node.inverse.body[0].type === 'GlimmerBlockStatement' && isIf && _astNodeInfo.default.isIf(node.inverse.body[0])) {
