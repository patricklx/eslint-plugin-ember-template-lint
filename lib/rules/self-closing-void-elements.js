...
class SelfClosingVoidElements extends _base.default {
  parseConfig(config) {
    ...
    let errorMessage = (0, _createErrorMessage.default)(this.ruleName, ['  * boolean - `true` to enable and `false` to disable', '  * "require" - checks for closing tags in void elements'], config);
      ...
      GlimmerElementNode(node) {
        if (VOID_TAGS[node.tag]) {
          ...
          if (shouldLogError) {
            ...
            if (this.mode === 'fix') {
              let editedNode = _emberTemplateRecast.builders.element(node.tag, {
...
exports.default = SelfClosingVoidElements;