...
const lintConfigs = syncFn({ config: templateLintConfig });
...
delete lintConfigs.configs.recommended.overrides;

Object.values(lintConfigs.configs).forEach((config) => {
  const rules = {};
  Object.entries(config.rules).forEach(([rule, conf]) => {
    rules['ember-template/' + rule] = conf;
  });
  config.rules = rules;
})

module.exports = {
  configs: lintConfigs.configs,
  rules: createRules(lintConfigs.rules)
}