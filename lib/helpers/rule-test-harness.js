...
exports.default = generateRuleTests;
var _nodeAssert = _interopRequireDefault(require("node:assert"));
var _getConfig = require("../get-config.js");
var _linter = _interopRequireDefault(require("../linter.js"));
...
const ALLOWED_GENERATE_RULE_TEST_PROPERTIES = new Set(['bad', 'config', 'error', 'focusMethod', 'good', 'groupMethodBefore', 'groupingMethod', 'meta', 'name', 'plugins', 'skipDisabledTests', 'testMethod']);
const ALLOWED_TEST_CASE_PROPERTIES_GOOD = new Set(['config', 'meta', 'name', 'template']);
const ALLOWED_TEST_CASE_PROPERTIES_BAD = new Set(['config', 'fixedTemplate',
// Only in bad test cases.
'meta', 'name', 'result', 'results', 'template', 'verifyResults', 'skipDisabledTests']);
const ALLOWED_TEST_CASE_PROPERTIES_ERROR = new Set(['config', 'meta', 'name', 'result', 'results', 'template', 'verifyResults']);
const ALLOWED_RESULT_PROPERTIES_BAD = new Set(['column', 'endColumn', 'endLine', 'filePath', 'isFixable',
// Only in bad test cases.
'line', 'message', 'rule', 'severity', 'source']);
const ALLOWED_RESULT_PROPERTIES_ERROR = new Set(['column', 'endColumn', 'endLine', 'fatal',
// Only in error test cases.
'filePath', 'line', 'message', 'rule', 'severity', 'source']);
function validateKeys(instance, allowedKeys, message) {
  for (const key of Object.keys(instance)) {
    (0, _nodeAssert.default)(allowedKeys.has(key), `${message}: ${key}. Expected one of: ${[...allowedKeys.keys()].join(', ')}.`);
  }
}
function validateInput(args) {
  // Validate arguments.
  (0, _nodeAssert.default)(args.length === 1, '`generateRuleTests` should only be called with one argument.');
  (0, _nodeAssert.default)(typeof args === 'object', '`generateRuleTests` should only be called with an object argument.');
  const [properties] = args;

  // Validate top-level properties.
  validateKeys(properties, ALLOWED_GENERATE_RULE_TEST_PROPERTIES, `Unexpected property passed to \`generateRuleTests\``);

  // Validate good test cases.
  for (const _item of properties.good || []) {
    let item = typeof _item === 'string' ? {
      template: _item
    } : _item;
    validateKeys(item, ALLOWED_TEST_CASE_PROPERTIES_GOOD, `Unexpected property passed to good test case`);
  }

  // Validate bad test cases.
  for (const item of properties.bad || []) {
    validateKeys(item, ALLOWED_TEST_CASE_PROPERTIES_BAD, `Unexpected property passed to bad test case`);
    (0, _nodeAssert.default)(!(item.result && item.results), 'Bad test case should not have both `result` and `results`.');
    let results = item.results || (item.result ? [item.result] : []);
    for (const result of results) {
      validateKeys(result, ALLOWED_RESULT_PROPERTIES_BAD, `Unexpected property passed to bad test case results`);
    }
  }

  // Validate error test cases.
  for (const item of properties.error || []) {
    validateKeys(item, ALLOWED_TEST_CASE_PROPERTIES_ERROR, `Unexpected property passed to error test case`);
    (0, _nodeAssert.default)(!(item.result && item.results), 'Error test case should not have both `result` and `results`.');
    let results = item.results || (item.result ? [item.result] : []);
    for (const result of results) {
      validateKeys(result, ALLOWED_RESULT_PROPERTIES_ERROR, `Unexpected property passed to error test case results`);
    }
  }
}
 ...
 */
function generateRuleTests(...args) {
  validateInput(args);
  const [{
    bad = [],
    good = [],
  ...
  groupingMethod(name, function () {
     ...
     */
    const prepare = _async(function (item, localConfig) {
      let template = typeof item === 'string' ? item : item.template;
      let meta = parseMeta(item);
      return _await(updateLinterConfig(localConfig), function () {
        linter.workingDir = meta.workingDir;
        return getVerifyOptions(template, meta);
      });
    });
    const updateLinterConfig = _async(function (localConfig) {
      if (localConfig === undefined) {
        return;
      }
      return _await(linter.getConfig(), function (config) {
        config.rules[name] = localConfig;
        config.rules = (0, _getConfig.processRules)(config);
      });
    });
    ...
    function checkLinterReuse() {
      if (usedLinters.has(linter)) {
        throw new Error('ember-template-lint: Test harness found invalid setup (`groupingMethodBefore` not called once per test). Maybe you meant to pass `groupingMethodBefore: beforeEach`?');
      }
    ...
    function checkDuplicateTestCase(item, seenTestCases, testCaseType) {
      ...
      (0, _nodeAssert.default)(!seenTestCases.has(serializedTestCase), `detected duplicate \`${testCaseType}\` test case`);
      seenTestCases.add(serializedTestCase);
    }
    const seenBadTestCases = new Set();
    for (const item of bad) {
      let template = item.template;
      let shouldRunDisabledTests = 'skipDisabledTests' in item ? !item.skipDisabledTests : !skipDisabledTests;
      ...
      testOrOnly(`${testName}: logs errors`, _async(function () {
        checkLinterReuse();
        checkDuplicateTestCase(item, seenBadTestCases, 'bad');
        return _await(prepare(item, item.config), function (options) {
          return _await(linter.verify(options), function (actual) {
            (0, _nodeAssert.default)(actual.length > 0, '`bad` test cases should always emit at least one failure');
            for (const violation of actual) {
              if (violation.fix) {
                (0, _nodeAssert.default)(typeof item.fixedTemplate === 'string', 'fixable test cases must assert the `fixedTemplate`');
              }
            }
            if (actual.every(violation => !violation.fix)) {
              (0, _nodeAssert.default)(!item.fixedTemplate, 'non-fixable test cases should not provide `fixedTemplate`');
            }
            (0, _nodeAssert.default)(item.template !== item.fixedTemplate, 'Test case `template` should not equal the `fixedTemplate`');
            if (item.fatal) {
              _nodeAssert.default.strictEqual(actual.length, 1); // can't have more than one fatal error
              delete actual[0].source; // remove the source (stack trace is not easy to assert)
              _nodeAssert.default.deepStrictEqual(actual[0], item.fatal);
              ...
              if (!('result' in item) && !('results' in item)) {
                ...
                (0, _nodeAssert.default)(hasRequiredProps, `\`bad\` test cases without "result" or "results" property supported only for template fixes with shape like { template: string, fixedTemplate: string }, given: ${JSON.stringify(item)}`);
              }
              ...
              if (results.length) {
                ...
                let expectedResults = results.map((result, index) => parseResult(result, actual[index]));
                _nodeAssert.default.deepStrictEqual(actual, expectedResults);
              }
            }
          });
        });
      ...
      if (shouldRunDisabledTests) {
        testOrOnly(`${testName}: passes when rule is disabled`, _async(function () {
          checkLinterReuse();
          let config = false;
          return _await(prepare(item, config), function (options) {
            return _await(linter.verify(options), function (actual) {
              _nodeAssert.default.deepStrictEqual(actual, []);
            });
          });
        }));
        testOrOnly(`${testName}: passes when disabled via inline comment - single rule`, _async(function () {
          checkLinterReuse();
          return _await(prepare(`${DISABLE_ONE}\n${template}`), function (options) {
            return _await(linter.verify(options), function (actual) {
              _nodeAssert.default.deepStrictEqual(actual, []);
            });
          });
        }));
        testOrOnly(`${testName}: passes when disabled via long-form inline comment - single rule`, _async(function () {
          checkLinterReuse();
          return _await(prepare(`${DISABLE_ONE_LONG}\n${template}`), function (options) {
            return _await(linter.verify(options), function (actual) {
              _nodeAssert.default.deepStrictEqual(actual, []);
            });
          });
        }));
        testOrOnly(`${testName}: passes when disabled via inline comment - all rules`, _async(function () {
          checkLinterReuse();
          return _await(prepare(`${DISABLE_ALL}\n${template}`), function (options) {
            return _await(linter.verify(options), function (actual) {
              _nodeAssert.default.deepStrictEqual(actual, []);
            });
          });
      ...
      if (item.fixedTemplate) {
        testOrOnly(`\`${item.template}\` -> ${item.fixedTemplate}\``, _async(function () {
          checkLinterReuse();
          return _await(prepare(item, item.config), function (options) {
            return _await(linter.verifyAndFix(options), function (result) {
              if (item.template === item.fixedTemplate) {
                _nodeAssert.default.ok(result[0].output === undefined);
                return;
              }
              _nodeAssert.default.deepStrictEqual(result[0].output, item.fixedTemplate);
            });
          });
        }));
        testOrOnly(`${item.fixedTemplate}\` is idempotent`, _async(function () {
          checkLinterReuse();
          return _await(prepare(item, item.config), function (options) {
            options.source = item.fixedTemplate;
            return _await(linter.verifyAndFix(options), function (result) {
              _nodeAssert.default.equal(result[0].output, undefined);
            });
          });
        }));
      }
    ...
    for (const _item of good) {
      ...
      testOrOnly(`${testName}: passes`, _async(function () {
        checkLinterReuse();
    ...
    for (const item of error) {
      ...
      testOrOnly(`${testName}: errors with config \`${friendlyConfig}\``, _async(function () {
        checkLinterReuse();
        checkDuplicateTestCase(item, seenErrorTestCases, 'error');
        return _await(prepare(item, item.config), function (options) {
          return _await(linter.verify(options), function (actual) {
            let expectedResults = item.results || [item.result];
            expectedResults = expectedResults.map((result, index) => parseResult(result, actual[index]));
            for (const [i, element] of actual.entries()) {
              if (element.fatal) {
                delete expectedResults[i].rule;
                delete element.source;
                _nodeAssert.default.ok(element.message.includes(expectedResults[i].message));
