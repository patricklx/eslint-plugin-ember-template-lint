...
function validateKeys(instance, allowedKeys, message) {
  for (const key of Object.keys(instance)) {
    _nodeAssert(allowedKeys.has(key), `${message}: ${key}. Expected one of: ${[...allowedKeys.keys()].join(', ')}.`);
  }
}
function validateInput(args) {
  // Validate arguments.
  _nodeAssert(args.length === 1, '`generateRuleTests` should only be called with one argument.');
  _nodeAssert(typeof args === 'object', '`generateRuleTests` should only be called with an object argument.');
  ...
  for (const item of properties.bad || []) {
    ...
    _nodeAssert(!(item.result && item.results), 'Bad test case should not have both `result` and `results`.');
  ...
  for (const item of properties.error || []) {
    ...
    _nodeAssert(!(item.result && item.results), 'Error test case should not have both `result` and `results`.');
    let results = item.results || (item.result ? [item.result] : []);
    for (const result of results) {
...
function generateRuleTests(...args) {
  ...
  groupingMethod(name, function () {
    ...
    function checkDuplicateTestCase(item, seenTestCases, testCaseType) {
      ...
      _nodeAssert(!seenTestCases.has(serializedTestCase), `detected duplicate \`${testCaseType}\` test case`);
    ...
    for (const item of bad) {
      ...
      testOrOnly(`${testName}: logs errors`, async function () {
        ...
        _nodeAssert(actual.length > 0, '`bad` test cases should always emit at least one failure');
        for (const violation of actual) {
          if (violation.fix) {
            _nodeAssert(typeof item.fixedTemplate === 'string', 'fixable test cases must assert the `fixedTemplate`');
          }
        }
        if (actual.every(violation => !violation.fix)) {
          _nodeAssert(!item.fixedTemplate, 'non-fixable test cases should not provide `fixedTemplate`');
        }
        _nodeAssert(item.template !== item.fixedTemplate, 'Test case `template` should not equal the `fixedTemplate`');
        if (item.fatal) {
          _nodeAssert.strictEqual(actual.length, 1); // can't have more than one fatal error
          delete actual[0].source; // remove the source (stack trace is not easy to assert)
          _nodeAssert.deepStrictEqual(actual[0], item.fatal);
          ...
          if (!('result' in item) && !('results' in item)) {
            ...
            _nodeAssert(hasRequiredProps, `\`bad\` test cases without "result" or "results" property supported only for template fixes with shape like { template: string, fixedTemplate: string }, given: ${JSON.stringify(item)}`);
          }
          ...
          if (results.length) {
            ...
            _nodeAssert.deepStrictEqual(actual, expectedResults);
          }
        }
      });
      if (shouldRunDisabledTests) {
        testOrOnly(`${testName}: passes when rule is disabled`, async function () {
          ...
          let actual = await linter.verify(options);
          _nodeAssert.deepStrictEqual(actual, []);
        });
        testOrOnly(`${testName}: passes when disabled via inline comment - single rule`, async function () {
          checkLinterReuse();
          let options = await prepare(`${DISABLE_ONE}\n${template}`);
          let actual = await linter.verify(options);
          _nodeAssert.deepStrictEqual(actual, []);
        });
        testOrOnly(`${testName}: passes when disabled via long-form inline comment - single rule`, async function () {
          checkLinterReuse();
          let options = await prepare(`${DISABLE_ONE_LONG}\n${template}`);
          let actual = await linter.verify(options);
          _nodeAssert.deepStrictEqual(actual, []);
        });
        testOrOnly(`${testName}: passes when disabled via inline comment - all rules`, async function () {
          checkLinterReuse();
      ...
      if (item.fixedTemplate) {
        testOrOnly(`\`${item.template}\` -> ${item.fixedTemplate}\``, async function () {
          ...
          _nodeAssert.deepStrictEqual(result[0].output, item.fixedTemplate);
        });
        testOrOnly(`${item.fixedTemplate}\` is idempotent`, async function () {
          ...
          let result = await linter.verifyAndFix(options);
          _nodeAssert.deepStrictEqual(result[0].output, undefined);
        });
      }
    }
      ...
      testOrOnly(`${testName}: errors with config \`${friendlyConfig}\``, async function () {
        ...
        for (const [i, element] of actual.entries()) {
          if (element.fatal) {
            ...
            _nodeAssert.ok(element.message.includes(expectedResults[i].message));
