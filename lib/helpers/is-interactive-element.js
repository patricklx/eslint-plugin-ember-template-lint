...
var _astNodeInfo = _interopRequireDefault(require("./ast-node-info.js"));
...
function isAudioVideoWithControls(node) {
  return (node.tag === 'audio' || node.tag === 'video') && _astNodeInfo.default.hasAttribute(node, 'controls');
}
function isHyperLink(node) {
  return node.tag === 'a' && _astNodeInfo.default.hasAttribute(node, 'href');
}
function isHiddenInput(node) {
  if (node.tag !== 'input') {
    return false;
  }
  let type = _astNodeInfo.default.findAttribute(node, 'type');
...
function reason(node) {
  ...
  if (_astNodeInfo.default.hasAttribute(node, 'tabindex')) {
    return 'an element with the `tabindex` attribute';
  }
  if ((node.tag === 'img' || node.tag === 'object') && _astNodeInfo.default.hasAttribute(node, 'usemap')) {
