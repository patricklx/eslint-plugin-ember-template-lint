...
exports.expandFileGlobs = expandFileGlobs;
exports.getFilesToLint = getFilesToLint;
exports.getPossibleOptionNames = getPossibleOptionNames;
...
var _extractTemplates = require("../extract-templates.js");
...
function getVersion() {
  const json = JSON.parse(_nodeFs.default.readFileSync(new URL('../../package.json', require('url').pathToFileURL(__filename).toString())));
  return json.version;
}
function parseArgv(_argv) {
  const specifiedOptions = {
    ...
    config: {
      describe: 'Define a custom configuration to be used - (e.g. \'{ "rules": { "no-implicit-this": "error" } }\') ',
      type: 'string'
    },
    quiet: {
    ...
    rule: {
      describe: 'Specify a rule and its severity to add that rule to loaded rules - (e.g. `no-implicit-this:error` or `rule:["error", { "allow": ["some-helper"] }]`)',
      type: 'string'
    },
    filename: {
  ...
  let parser = (0, _yargs.default)().scriptName('ember-template-lint').usage('$0 [options] [files..]').options(specifiedOptions).help().version('version', getVersion());
  ...
  if (_argv.length === 0) {
    ...
    for (const optionName of Object.keys(options)) {
      if (!['$0', '_'].includes(optionName) &&
      // Built-in yargs options.
...
function expandFileGlobs(workingDir, filePatterns, ignorePattern, glob = executeGlobby) {
  ...
  for (const pattern of filePatterns) {
    let isLiteralPath = !(0, _isGlob.default)(pattern) && isFile(_nodePath.default.resolve(workingDir, pattern));
    if (isLiteralPath) {
      // If `--no-ignore-pattern` is passed, the ignorePatter is `[false]`.
      let isIgnored = !(ignorePattern?.length === 1 && ignorePattern[0] === false) && _micromatch.default.isMatch(pattern, ignorePattern);
      if (!isIgnored) {
        result.add(pattern);
      }
      continue;
...
function getPossibleOptionNames(specifiedOptions) {
  const optionAliases = Object.values(specifiedOptions).map(option => option.alias).filter(option => option !== undefined);
  const dasherizedOptionNames = [...Object.keys(specifiedOptions), ...optionAliases];
  const camelizedOptionNames = dasherizedOptionNames.map(name => (0, _camelize.default)(name));
  const negatedDasherizedOptionNames = dasherizedOptionNames.map(name => name.startsWith('no-') ? name.slice(3) : `no-${name}`);
  const negatedCamelizedOptionNames = negatedDasherizedOptionNames.map(name => (0, _camelize.default)(name));
  return [...dasherizedOptionNames, ...camelizedOptionNames,
  // Since yargs `boolean-negation` option is enabled (by default), assume any option can be passed with `no`/negated prefix.
  ...negatedDasherizedOptionNames, ...negatedCamelizedOptionNames];
}
function executeGlobby(workingDir, pattern, ignore) {
  let supportedExtensions = new Set(['.hbs', '.handlebars', ..._extractTemplates.SUPPORTED_EXTENSIONS]);

  // `--no-ignore-pattern` results in `ignorePattern === [false]`
  let options = ignore[0] === false ? {
    cwd: workingDir
  } : {
    cwd: workingDir,
    gitignore: true,
  ...
  return (0, _globby.globbySync)(pattern, options).filter(filePath => supportedExtensions.has(_nodePath.default.extname(filePath)));
}
function isFile(possibleFile) {
  try {
    let stat = _nodeFs.default.statSync(possibleFile);
    return stat.isFile();
  } catch {
    return false;
  }
