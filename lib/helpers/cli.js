...
function getVersion() {
  const json = JSON.parse(_nodeFs.readFileSync(new URL('../../package.json', import.meta.url)));
  return json.version;
}
function parseArgv(_argv) {
  ...
  let parser = _yargs().scriptName('ember-template-lint').usage('$0 [options] [files..]').options(specifiedOptions).help().version('version', getVersion());
...
function expandFileGlobs(workingDir, filePatterns, ignorePattern, glob = executeGlobby) {
  ...
  for (const pattern of filePatterns) {
    let isLiteralPath = !_isGlob(pattern) && isFile(_nodePath.resolve(workingDir, pattern));
    if (isLiteralPath) {
      // If `--no-ignore-pattern` is passed, the ignorePatter is `[false]`.
      let isIgnored = !(ignorePattern?.length === 1 && ignorePattern[0] === false) && _micromatch.isMatch(pattern, ignorePattern);
      if (!isIgnored) {
        result.add(pattern);
      }
      continue;
...
function getPossibleOptionNames(specifiedOptions) {
  ...
  const camelizedOptionNames = dasherizedOptionNames.map(name => _camelize(name));
  const negatedDasherizedOptionNames = dasherizedOptionNames.map(name => name.startsWith('no-') ? name.slice(3) : `no-${name}`);
  const negatedCamelizedOptionNames = negatedDasherizedOptionNames.map(name => _camelize(name));
...
function executeGlobby(workingDir, pattern, ignore) {
  ...
  return (0, _globby.globbySync)(pattern, options).filter(filePath => supportedExtensions.has(_nodePath.extname(filePath)));
}
function isFile(possibleFile) {
  try {
    let stat = _nodeFs.statSync(possibleFile);
    return stat.isFile();
  } catch {
    return false;
  }
