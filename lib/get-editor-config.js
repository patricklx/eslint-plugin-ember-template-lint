...
class EditorConfigResolver {
  ...
  opts(filepath, options) {
    const resolvedFilePath = _nodePath.default.resolve(filepath);
    return [resolvedFilePath, this.processOptions(options, resolvedFilePath)];
  }
  processOptions(options = {}, filepath) {
    return {
      config: options.config || '.editorconfig',
      root: _nodePath.default.resolve(options.root || _nodePath.default.parse(filepath).root)
    };
  }
  parseString(data) {
    let sectionBody = {};
  ...
  buildFullGlob(pathPrefix, glob) {
    ...
    return _nodePath.default.join(pathPrefix, glob);
  }
  fnmatch(filepath, glob) {
    ...
    glob = glob.split(_nodePath.default.sep).join('/');
    filepath = filepath.split(_nodePath.default.sep).join('/');
    return _micromatch.default.isMatch(filepath, glob, matchOptions);
  }
  ...
  readConfigFilesSync(filepaths) {
    ...
    for (const filepath of filepaths) {
      ...
      let file;
      try {
        file = _nodeFs.default.readFileSync(filepath, 'utf8');
      } catch {
        file = '';
      }
      files.push({
  ...
  processMatches(matches) {
    ...
    if ('indent_style' in matches && matches.indent_style === 'tab' && !('indent_size' in matches)) {
  ...
  parseFromConfigs(configs, filepath) {
    return this.processMatches(configs.reverse().reduce((matches, file) => {
      const pathPrefix = _nodePath.default.dirname(file.name);
  ...
  getConfigFileNames(filepath, options) {
    const paths = [];
    do {
      filepath = _nodePath.default.dirname(filepath);
      paths.push(_nodePath.default.join(filepath, options.config));
  ...
  */
  resolveEditorConfigFiles(_filepath = _nodePath.default.join(this.workingDir, 'this.file.does.not.exist'), _options = {}) {
  ...
  getEditorConfigData(entry, _opts = {}) {
    ...
    const resolvedFilePath = _nodePath.default.resolve(entry);
    return this.cachedEditorConfigForFilePath(resolvedFilePath);
  }
}
exports.default = EditorConfigResolver;
